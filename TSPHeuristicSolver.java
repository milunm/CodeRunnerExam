...
public class TSPHeuristicSolver {
    ...
    // Q6 
    public static ArrayList<Integer> solveTSP_hybrid(int numCities, double[][] matrix, int iterations) {
        if (numCities <= 1 || isValidMatrix(matrix) == 0 || iterations < 1) return null; //validation before proceeding (check if invalid total cities, invalid matrix, or not enough iterations)
//this approach uses a combination of simulated annealing and hill climbing
        ArrayList<Integer> currentTour = generateInitialTour(numCities); //generate a random valid tour
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (i >= iterations - hillClimbCutoff){ //use hill climbing in final iterations
                if (newFitness < currentFitness){
                    currentTour = newTour;
                    currentFitness = newFitness; //assign new current tour and fitness values if chosen
                }
            }
            //use simulated annealing in earlier iterations
            else if (fitnessChange < 0 || Math.exp(-fitnessChange / temperature) > Math.random()){ //sometimes accept worse solutions based on probability generated by temperature + the fitness difference 
                //(higher probability if tour is only slightly worse, if a lot worse then low probability of accepting.) 
                currentTour = newTour;
                currentFitness = newFitness; //assign new values based on annealing logic
            }

            //cool temperature after each iteration, reduces chances of worse solutions being chosen. ensures the algorithm doesnt get stuck at a local minimum at the beginning iterations
            temperature *= (1 - rateOfCooling);
        }

        return currentTour; //RETURN THE FINAL TOUR
    }
    ...
    public static ArrayList<Integer> solveTSP_SA(int numCities, double[][] matrix, int iterations){
        ...
        double rateOfCooling = 0.995; //cooling rate (enables cooling over each iteration)
        for (int i = 0; i < iterations; i++){
            ...
            if (fitnessChange < 0 || Math.exp(-fitnessChange / temperature) > Math.random()){ //sometimes accept worse solutions based on probability generated by temperature + the fitness difference 
                //(higher probability if tour is only slightly worse, if a lot worse then low probability of accepting.) 
    ...
    public static ArrayList<Integer> solveTSP_HC(int numCities, double[][] matrix, int iterations){
        if (numCities <= 1 || isValidMatrix(matrix) == 0 || iterations < 1) return null; //validation before proceeding 
        ...
        for (int i = 0; i < iterations; i++){
            ...
            if (fitnessChange < 0){ //if new fitness is better, select this as the best found tour and progress 
